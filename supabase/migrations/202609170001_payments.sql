...
begin
  ...
  select p.id, p.name, i->>'weight', i->>'grind', (i->>'quantity')::integer,
    case i->>'weight' when '150g' then p.price_150g when '300g' then p.price_300g end,
    (case i->>'weight' when '150g' then p.price_150g when '300g' then p.price_300g end) * (i->>'quantity')::integer
  from jsonb_array_elements(p_items) i
  join public.products p on p.id = (i->>'product_id')::uuid and p.active
  where i->>'weight' in ('150g', '300g') and i->>'grind' in ('Whole Bean', 'Filter', 'Espresso')
